...
args="D:/Cache/21783/207402/229016 D:/Cache/21783/207402/229016/379173 max calcpeaks-simple-v1-4"
...
dataout_ = list()
peaksout_ = list()
for(p in 1:length(unique(data$data_collection_id))){
  
  dataIn = data[which(data$data_collection_id==unique(data$data_collection_id)[p]),]
  
  #reduce the data to the minimum modifed to reproduce the peaks. 
  min_mod = unique(dataIn$modified)[which.min(as.POSIXct(unique(dataIn$modified)))]
  
  data_held_out_status = FALSE
  
  if(length(unique(dataIn$modified)) > 1){
    data_held_out = dataIn[-which(dataIn$modified==min_mod),]
    dataIn = dataIn[which(dataIn$modified==min_mod),]
    data_held_out_status = TRUE
  }

  #assume the data has all been reviewed by the user running script
  dataIn[which(!is.na(dataIn$probability)),"analyst"] = as.integer(dbFetch(dbSendQuery(con,"SELECT id FROM personnel WHERE personnel.pg_name = current_user"))$id)
  
  sfs = data.frame(rbind(dataIn[,c("StartFile","data_collection_id")],setNames(dataIn[,c("EndFile","data_collection_id")],c("StartFile","data_collection_id"))))
  ...
  dataIn = merge(dataIn,metadata,"StartFile")
  
  dataIn$hourly<-format(dataIn$datetime,"%y%m%d %H")
  
  #query this... 
  
  SecInPng=dbFetch(dbSendQuery(con,paste("SELECT length_seconds FROM bin_type_codes JOIN signals ON bin_type_codes.id = signals.native_bin WHERE signals.id =",dataIn$signal_code[1])))$length_seconds
  
  total_pngs = dataIn[which(!duplicated(dataIn$id.y)),]
  ...
  total_neg = aggregate((EndTime - StartTime) ~ hourly,data = dataIn[which(is.na(dataIn$probability)),],sum)
  ...
  peakTimes<-foreach(f=1:length(MaxHrs)) %do% {
    ...
    Datedata<-dataIn[which(dataIn$hourly %in% hourly_perc$hourly[IDvec==f] & !is.na(dataIn$probability)),]
    
    countsInPeak<-nrow(Datedata)            
    if(calc_type=="max"){
      Peak<-Datedata[which.max(Datedata[,"probability"]),]
    }else if(calc_type=="med"){
      Peak<-Datedata[whichmedian(Datedata[,"probability"]),]
    }else{
      stop("value not supported for calc_type")
    }
    
    
    dataIn[which(dataIn$hourly %in% hourly_perc$hourly[IDvec==f] & !is.na(dataIn$probability)),"assoc_id"]=Peak$id.x
  ...
  peaksout = data.frame(peakTimes$StartTime,peakTimes$EndTime,peakTimes$LowFreq,peakTimes$HighFreq,
                        ...
                        peakTimes$b,peakTimes$id.x,peakTimes$countsInPeak)
  ...
  colnames(peaksout)=c('StartTime','EndTime','LowFreq','HighFreq','StartFile',
                       ...
                       'original_id','date_created',"data_collection_id",'id',"countsInPeak")
  
  
  #reset the labels
  #peaksout$label = ""
  
  dataout = data.frame(dataIn$StartTime,dataIn$EndTime,dataIn$LowFreq,dataIn$HighFreq,
                       dataIn$StartFile,dataIn$EndFile,dataIn$probability,dataIn$comments,
                       dataIn$procedure,dataIn$label,dataIn$signal_code,dataIn$strength,
                       dataIn$modified,dataIn$analyst,dataIn$status,dataIn$original_id,dataIn$date_created,
                       dataIn$b,dataIn$id.x,dataIn$assoc_id)
  
  colnames(dataout)=c('StartTime','EndTime','LowFreq','HighFreq','StartFile',
                       'EndFile','probability','comments','procedure','label',
                       'signal_code','strength','modified','analyst','status',
                       'original_id','date_created',"data_collection_id",'id',"peak_assoc_id")
  
  #combine with held out data
  
  if(data_held_out_status){
    
    data_held_out = data_held_out[,which(colnames(data_held_out) %in% colnames(dataout))]
    
    data_held_out$peak_assoc_id=-1
    
    dataout = rbind(dataout,data_held_out)
  }
  
  dataout_[[p]] = dataout
  peaksout_[[p]] = peaksout
}
...
write.csv(do.call('rbind',peaksout_),gzfile(paste(resultPath,"DETx.csv.gz",sep="/")),row.names = FALSE)
write.csv(do.call('rbind',dataout_),gzfile(paste(resultPath,"DETx2.csv.gz",sep="/")),row.names = FALSE)

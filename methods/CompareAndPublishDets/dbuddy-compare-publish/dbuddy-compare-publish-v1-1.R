...
if(length(mod_keys)>0){
  ...
  if(nrow(EditMod)>0){
    ...
    filepath = paste(transferpath,"ModTemp.csv",sep="/")
    logpath = paste(transferpath,"temp1.txt",sep="/")
...
if(nrow(new_data)>0){
  ...
  filepath = paste(transferpath,"NewTemp.csv",sep="/")
  logpath = paste(transferpath,"temp2.txt",sep="/")
...
if(length(del_keys)>0){
  filepath = paste(transferpath,"DelKeysTemp.csv",sep="/")
  logpath = paste(transferpath,"temp3.txt",sep="/")

#fork of train-win-slide-dl-v1-21. Leave contrast bug in place for better 1
#to one comparison with prod model. 
...
if stage !="train":
    ...
    mixup_alpha = float(args[10])
    augstr = args[11].split(",")
    brightness_high=float(augstr[0])
    brightness_low=float(augstr[1])
    contrast_high =float(augstr[2])
    do_aug = args[12]
    do_spec_averaging = args[13]
    do_val = args[14]
    do_weight = args[15]
    GT_depth = args[16].count(",")+1
    learning_rate = float(args[17])
    model_name = args[18]
    native_img_height = int(args[19])
    native_pix_per_sec = int(args[20])
    fp_perc= float(args[21])
    tp_perc= float(args[22])
    prop_fp= float(args[23])
    prop_tp= float(args[24])
    shuf_size = float(args[25])
    stride_pix_train = int(args[26]) 
    tp_weights = float(args[27])
    view_plots = args[28]
    model_win_size = int(args[29])
    win_size_native = int(args[30])
    zero_drop = args[31]
    #arguments:
    val_steps = int(args[33])
    val_tp_heavy = args[34]
    batch_size_train = int(args[35])
    batch_size = int(args[36])
    epochs = int(args[37])
    epoch_steps = int(args[38])
...
def MakeDatasetTest(dataset,wh,wl,stride_pix,do_offset=False):
...
#@tf.function
def pao_mixup(ds1,ds2,addnoise_mean,addnoise_stddev,weights):
...
#little weird but avoids dependency conflict
@tf.function
def mixup(ds1,ds2,alpha):
    
    alpha_tensor = tf.cast(alpha, tf.float32)
    
    # 1. Sample from two Gamma distributions
    # tf.random.gamma implicitly uses a scale (beta/theta) of 1.0
    gamma1 = tf.random.gamma(shape=[], alpha=alpha_tensor)
    gamma2 = tf.random.gamma(shape=[], alpha=alpha_tensor)
    
    # Calculate lambda to simulate Beta(alpha, alpha)
    # Adding a tiny epsilon to the denominator prevents potential division by zero
    ...
    #xb_f = tf.cast(xb, tf.float32)
    
    #y_f = tf.cast(tf.cast(y,tf.int32), tf.float32)
    #yb_f = tf.cast(tf.cast(yb,tf.int32), tf.float32)
    
    lam_f = tf.cast(lam, tf.float32)

    # 3. Apply the Mixup formula
    x_s = lam_f * x + (1.0 - lam_f) * xb
    y_s = lam_f * tf.cast(y, tf.float32) + (1.0 - lam_f) * tf.cast(yb,tf.float32)
...
def accumulate_lab(x,y,z,z2):
    ...
    weight = tf.cast(out_tens_tp_prop,tf.float32)

    #else:
        #out_tens = tf.cast(y,tf.float32)
        #weight = tf.constant(0.)


    return x,out_tens,z,weight
...
def ingest(x,y,z,z1,wh,wl,stride_pix,do_offset): #try out a predetermined offset
    ...
    #image =  tf.slice(image,[0,0,0,0],[img_shape[0], img_shape[1], img_shape[2],1]) #take whole tensor except for last item of batch
    ...
    #one idea is to just create the size string i'll need: but probably most effecient to keep it small
    #lab= tf.constant(int(lab[0]),shape=[int(img_shape[])*GT_depth])

    if int(tf.shape(lab)[0])!=1:
        ...
        ##lab = tf.constant(lab[0],shape=[native_img_height,round(wl/win_t_factor),GT_depth])
        lab = tf.repeat(lab,int(img_shape[0])*native_img_height*round(wl/win_t_factor)*GT_depth)
        lab = tf.reshape(lab,[int(img_shape[0]),native_img_height,round(wl/win_t_factor),1])
    ...
    if int(tf.shape(splt)[0])!=1:
        ...
        ##splt = tf.constant(int(splt[0]),shape=[1,round(wl/win_t_factor),1])
        splt = tf.repeat(splt,int(img_shape[0])*round(wl/win_t_factor))
        splt = tf.reshape(splt,[int(img_shape[0]),1,round(wl/win_t_factor),1])
        
    
    #if splt.shape[2]!=lab.shape[2]:
    #    import code
    #   code.interact(local=dict(globals(), **locals()))

    
    return image,lab,splt,tf.zeros(img_shape[0])
...
if stage=="train":
    ...
    #iter_obj = iter(MakeDatasetTest(full_dataset,model_win_size,model_win_size,stride_pix_train,True))
    ...
    train_dataset_noise = MakeDataset(full_dataset,model_win_size,model_win_size,stride_pix_train,True,1,None,do_shuffle,True,None,False,None,0,True,do_weight_,False,zero_drop)
    ...
    train_dataset_signal = MakeDataset(TP_dataset,model_win_size,model_win_size,stride_pix_train,True,1,None,round(shuffle_size_all/20),\
                                       True,None,False,True,1,True,do_weight_,False)#round(shuffle_size_all/20) #iter(train_dataset_2gs)

    train_dataset_all = MakeDataset(full_dataset,model_win_size,model_win_size,stride_pix_train,True,1,None,round(shuffle_size_all/20),\
                                       True,None,False,True,None,True,do_weight_,False)
    if do_spec_averaging=="y":
        #this averages signal with noise based on function protocol
        train_dataset_signal2 = tf.data.Dataset.zip((train_dataset_signal, train_dataset_noise)).map(lambda ds1,ds2: (pao_mixup(ds1,ds2,addnoise_mean,addnoise_stddev,do_weight_))) #add averagethesefxn
        
        #this averages noise with noise based on function protocol
        train_dataset_noise_aug_w_noise = tf.data.Dataset.zip((train_dataset_noise, train_dataset_noise)).map(lambda ds1,ds2: (pao_mixup(ds1,ds2,addnoise_mean,addnoise_stddev,do_weight_)))
    ...
    elif do_spec_averaging=="pao_mixup":
        
        #positives will be half and half noise superimposed and half normal
        train_dataset_signal2 = tf.data.Dataset.zip((train_dataset_signal, train_dataset_noise)).map(lambda ds1,ds2: (pao_mixup(ds1,ds2,addnoise_mean,addnoise_stddev,do_weight_)))

        train_dataset = tf.data.Dataset.sample_from_datasets([train_dataset_signal2, train_dataset_noise], weights=[prop_tp, prop_fp])

    elif do_spec_averaging=="mixup":

        #so in mixup don't control the ratio as tightly (another engineering distinction). Let's try one with just mixup. 
        train_dataset = tf.data.Dataset.zip((train_dataset_all, train_dataset_all)).map(lambda ds1,ds2: (mixup(ds1,ds2,mixup_alpha)))

    elif do_spec_averaging=="kather":
        
        #positives will be half and half noise superimposed and half normal
        train_dataset_signal2 = tf.data.Dataset.zip((train_dataset_signal, train_dataset_noise)).map(lambda ds1,ds2: (kather(ds1,ds2)))

        train_dataset = tf.data.Dataset.sample_from_datasets([train_dataset_signal2, train_dataset_noise], weights=[prop_tp, prop_fp])
        
    elif do_spec_averaging=="n":
        #this is without the spectrogram averaging
        train_dataset = tf.data.Dataset.sample_from_datasets([train_dataset_signal, train_dataset_noise], weights=[prop_tp, prop_fp])
...
if stage=="test":
    ...
    #remove spectrograms after inference to conserve disk space. only use for scaling out deployments, not for eval.

    #if remove_spec=='y':
    #    import code
    #    code.interact(local=dict(globals(), **locals()))

        #remove both the individual files and filepaths.csv

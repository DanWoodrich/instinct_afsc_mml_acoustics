...
if stage !="train":
    ...
    learning_rate = float(args[12])
    model_name = args[13]
    native_img_height = int(args[14])
    native_pix_per_sec = int(args[15])
    fp_perc= float(args[16])
    tp_perc= float(args[17])
    prop_fp= float(args[18])
    prop_tp= float(args[19])
    shuf_size = float(args[20])
    stride_pix_train = int(args[21]) 
    tp_weights = float(args[22])
    view_plots = args[23]
    model_win_size = int(args[24])
    win_size_native = int(args[25])
...
def MakeDataset(dataset,wh,wl,stride_pix,do_offset=False,split=None,batchsize=None,shuffle_size=None,drop_assignment=True,\
    ...
    dataset = dataset.map(lambda x,y,z,z2: (ingest(x,y,z,z2,wh,wl,stride_pix,do_offset,shuffle_size)),num_parallel_calls = tf.data.AUTOTUNE).unbatch() #test, with offsets implemented in loop within ingest. 
    #accumulate label
...
@tf.function
def ingest(x,y,z,z1,wh,wl,stride_pix,do_offset,do_shuffle): #try out a predetermined offset
    ...
    #v1-14: do a shuffle in here 
    
    if do_shuffle!=None:
        inds = tf.random.shuffle(range(splt_shape[0]))
        
        image=tf.gather(image,inds)
        lab=tf.gather(lab,inds)
        splt=tf.gather(splt,inds)
...
if stage=="train":
    ...
    #temporarily setting shuffle to 111 (arbitrary value) to test changed shuffle behavior w/o breaking things too much. 
    train_dataset_noise = MakeDataset(full_dataset,model_win_size,model_win_size,stride_pix_train,True,1,None,111,True,None,False,None,0,True,True,False,zero_drop)

    #this produces signal, in grayscale
    train_dataset_signal = MakeDataset(TP_dataset,model_win_size,model_win_size,stride_pix_train,True,1,None,111,\
    ...
    train_dataset_signal_aug_w_noise = tf.data.Dataset.zip((train_dataset_signal, train_dataset_noise)).map(lambda ds1,ds2: (add_noise2(ds1,ds2,addnoise_mean,addnoise_stddev))) #add averagethesefxn
    ...
    train_dataset_noise_comb= tf.data.Dataset.sample_from_datasets([train_dataset_noise, train_dataset_noise_aug_w_noise], weights=[0.75, 0.25])
    
    #this samples from noise and signal datasets, and provides final augmentation.
    train_dataset = tf.data.Dataset.sample_from_datasets([train_dataset_signal_aug_w_noise, train_dataset_noise_comb], weights=[prop_tp, prop_fp])\

...
if stage !="train":
    ...
    do_val = args[13]
    do_weight = args[14]
    GT_depth = args[15].count(",")+1
    learning_rate = float(args[16])
    model_name = args[17]
    native_img_height = int(args[18])
    native_pix_per_sec = int(args[19])
    fp_perc= float(args[20])
    tp_perc= float(args[21])
    prop_fp= float(args[22])
    prop_tp= float(args[23])
    shuf_size = float(args[24])
    stride_pix_train = int(args[25]) 
    tp_weights = float(args[26])
    view_plots = args[27]
...
def MakeDataset(dataset,wh,wl,stride_pix,do_offset=False,split=None,batchsize=None,shuffle_size=None,drop_assignment=True,\
    ...
    if shuffle_size!=None:
        dataset = dataset.shuffle(bigfile,reshuffle_each_iteration=True) #shuffle the whole thing

    if repeat_perma==True:
        dataset =dataset.repeat()

    dataset = dataset.interleave(lambda x,y,z,z2: tf.data.Dataset.from_tensors((x,y,z,z2)),cycle_length=32,block_length=1,num_parallel_calls = tf.data.AUTOTUNE,deterministic=False)

    dataset = dataset.map(lambda x,y,z,z2: (ingest(x,y,z,z2,wh,wl,stride_pix,do_offset)),num_parallel_calls = tf.data.AUTOTUNE,deterministic=False).prefetch(tf.data.AUTOTUNE).unbatch() #test, with offsets implemented in loop within ingest. 
...
#@tf.function
def add_noise2(ds1,ds2,addnoise_mean,addnoise_stddev,weights):
...
def ingest(x,y,z,z1,wh,wl,stride_pix,do_offset): #try out a predetermined offset
    ...
    out_tens = tf.reshape(lab,[GT_depth,-1])
    ...
    return image,out_tens,splt,weight

...
estbins =(sum(FGdurs)*native_pix_per_sec)/stride_pix
...
def MakeDataset(dataset,wh,wl,split=None,batchsize=20,do_shuffle=True,drop_assignment=True,augment=False,filter_splits=True):
    ...
    if data_augmentation=="NULL":
        augment=False

    dataset = dataset.map(lambda x,y,z,z2: (ingest(x,y,z,z2,wh,wl))).unbatch() #test, with offsets implemented in loop within ingest. 
    #accumulate label
...
def MakeDatasetOld(dataset,wh,wl,split=None,batchsize=20,do_shuffle=True,drop_assignment=True,augment=False,filter_splits=True):
    ...
    #dataset = dataset.filter(lambda x,y,z: z[0]== 0)
    ...
    #redo of this
    dataset = dataset.map(lambda x,y,z: (x,tf.where(tf.equal(y,1))[:,1],z)) #turn this into a tf slice to also work for multi-class
    ...
    if do_shuffle==True:
        dataset = dataset.shuffle(shuffle_size) #10000 #big number because why not?
    ...
    if split==1 or augment==True:
        dataset = dataset.map(lambda x,y,z: (add_guassian(x,Gaussian_val),y,z)) #tf.random.normal(tf.shape(x),0,tf.random.uniform([1],0,20)[0])
    ...
    #do augmentation on training or if not specified
    if split==1 or augment==True:
        #dataset = dataset.map(lambda x,y,z: (add_guassian(x,wh,wl),y,z))
        dataset = dataset.map(lambda x,y,z: (data_augmentation(x),y,z))

    #cast target to integer
    dataset = dataset.map(lambda x,y,z: (x,tf.cast(y, tf.int32),z))
    
    #drop assignment
    if drop_assignment==True:
        dataset = dataset.map(lambda x,y,z: (x,y))

    #need to add weights here anyways- look into adaptive weights
    #dataset = dataset.map(lambda x,y: (x,y,tp_weights))
...
#@tf.function
def ingest(x,y,z,z1,wh,wl): #try out a predetermined offset
...
if do_plot:
    ...
    iter_obj = iter(MakeDataset(full_dataset,win_height,win_length,_split,20,(shuffle=='y'),False,do_augment,do_filter))
    #next(iter_obj)
    #import code
    #code.interact(local=dict(globals(), **locals()))
    def seespec(obj):

...
def MakeDataset(dataset,wh,wl,stride_pix,do_offset=False,split=None,batchsize=None,shuffle_size=None,
                drop_assignment=True,addnoise=None,augment=False,filter_splits=True,label=None,
                repeat_perma=False,weights=True,zero_drop=None):
    ...
    if zero_drop!=None:
        dataset = dataset.filter(lambda x,y,z,z2: tf.reduce_all(float(y) > tf.random.uniform(shape=[],minval=-zero_drop,maxval=1-zero_drop)))
...
#def MakeDataset(dataset,wh,wl,stride_pix,do_offset=False,split=None,batchsize=None,shuffle_size=None,
    #drop_assignment=True,addnoise=None,augment=False,filter_splits=True,label=None,repeat_perma=False,weights=True,zero_drop=None):


    train_dataset = MakeDataset(full_dataset,model_win_size,model_win_size,stride_pix_train,True,1,batch_size_train,shuffle_size_all,True,None,True,True,None,True,True,0.99)

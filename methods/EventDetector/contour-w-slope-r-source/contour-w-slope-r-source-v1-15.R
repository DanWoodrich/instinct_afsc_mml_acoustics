...
EventDetectoR<-function(soundFile=NULL,spectrogram=NULL,dataMini=NULL,ParamArgs=NULL,verbose=NULL,metadata=NULL){
  ...
  MatrixDenoise <-ParamArgs[12] #row,column, or n
  noiseThresh<-as.numeric(ParamArgs[13]) #0.9
  noiseWinLength<-as.numeric(ParamArgs[14]) #2.5
  Overlap<-as.numeric(ParamArgs[15]) 
  pixThresh<-as.numeric(ParamArgs[16])#  100
  #t_samp_rate
  windowLength<-as.numeric(ParamArgs[18]) #
  ...
  }else if(MatrixDenoise=="column"){
    #otherwise, noise win length is in freq
    noiseWinLength
  ...
  if(DesiredSlope=="Stacked"){ #ignore slope for this option, in the future, can look at degrees of slope vs flat
    cont2<-cont[which(size>pixThresh & abs(slope)<=abs(houghSlopeMax)& abs(slope)>=abs(houghSlopeMin))] #ignore slopeMin for stacked. 

...
if(grepl('SELECT ',ParamArgs[which(ParamNames=="query")])){
  ...
  #check for overlap, stay in db format. 
  
  if(ParamArgs[which(ParamNames=="remove_proc_ovlp")]=='y' & nrow(GTdata)>1){
    
    start_fileDur_lookup = dbFetch(dbSendQuery(con,paste("SELECT id,duration FROM soundfiles WHERE id IN (",paste(unique(GTdata$start_file),collapse=",",sep=""),")",sep="")))
    
    GTdata$start_fileDur = start_fileDur_lookup$duration[match(GTdata$start_file,start_fileDur_lookup$id)]
    ...
    for(p in 1:length(unique(GTdata$signal_code))){
      ...
      GTdataIn$tempEnd = GTdataIn$end_time + GTdataIn$start_fileDur
      ...
      if(nrow(GTdataIn[which(GTdataIn$start_file==GTdataIn$end_file),])>0){
        
        GTdataInsamefile = GTdataIn[which(GTdataIn$start_file==GTdataIn$end_file),]
        GTdataIn[which(GTdataIn$start_file==GTdataIn$end_file),"midpoint"]= GTdataIn[which(GTdataIn$start_file==GTdataIn$end_file),"start_time"] + (GTdataIn[which(GTdataIn$start_file==GTdataIn$end_file),"end_time"] -GTdataIn[which(GTdataIn$start_file==GTdataIn$end_file),"start_time"])/2
        
        GTdataIn$real_end[which(GTdataIn$start_file==GTdataIn$end_file)] = GTdataInsamefile$end_time
      
      }
      
      if(nrow(GTdataIn[-which(GTdataIn$start_file==GTdataIn$end_file),])>0){
        GTdataIn[-which(GTdataIn$start_file==GTdataIn$end_file),"midpoint"]=GTdataIn[-which(GTdataIn$start_file==GTdataIn$end_file),"start_time"] + (GTdataIn[-which(GTdataIn$start_file==GTdataIn$end_file),"tempEnd"] -GTdataIn[-which(GTdataIn$start_file==GTdataIn$end_file),"start_time"])/2
        GTdataIn$real_end[-which(GTdataIn$start_file==GTdataIn$end_file)]= GTdataIn[-which(GTdataIn$start_file==GTdataIn$end_file),"tempEnd"]
      }
      ...
      for(i in 1:nrow(GTdataIn)){
        ...
        if(GTdataIn$delete[i]==0){
          ...
          for(n in 1:nrow(GTdataIn)){
            ...
            if(GTdataIn$start_time[n]<=mp & GTdataIn$real_end[n]>=mp & GTdataIn$delete[n]==0 & GTdataIn$procedure[n]!=GTdataIn$procedure[i] & GTdataIn$start_file[i] == GTdataIn$start_file[n]){
  ...
  #use GTdata_db_save so don't have to redo before. 
  
  #here, wrap all steps to convert to detx into a general function. 
  #db_to_detx <- function(conn,data)
  
  #all this has to do: 
  #lookup values of certain columns
  #change colnames to camel case
  
  all_lookup_cols = c("label","signal_code")

...
#v1-6: attempt to not use png (x11 reliant on linux) as this is crashing containerized version. 
library(tuneR)
library(signal)
library(imager)
library(doParallel)
...
img_print <-function(object,xbins,pix_height,path){
  ...
  #v1-6 change: add type = cairo
  png(path,height=pix_height,width=xbins,bg="black", type="cairo") #my problem is that upon saving, there is a background in 
...
args="../cache/379084 ../cache/379084/359062 ../temp/DecimatedWaves/256 BS24_AU_PM02 y 96 0 256 20 128 con_bright_no_rep-v1-6"

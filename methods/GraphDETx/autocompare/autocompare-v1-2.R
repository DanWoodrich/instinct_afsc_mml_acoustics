...
#V1-2: make colors default so lacking factor won't mess it up. 
...
p + facet_grid(rows = vars(newlab))+ theme(text=element_text(size=ggtextsize))+ guides(colour = guide_legend(override.aes = list(size=5))) + scale_color_manual(values = c("FP" = "#619CFF","TP"="#00BA38","FN"="#F8766D"))

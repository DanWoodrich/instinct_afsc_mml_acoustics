...
plot(curves[[unique(data$splits)[1]]], auc.main=FALSE, main ="",legend=FALSE,col=cols[unique(data$splits)[1]],cex.axis=1.8*cexmod,lwd = 3*cexmod)
...
for(i in unique(data$splits)){
  ...
  curvesub<-data.frame(curvesub$curve,i)
  colnames(curvesub)<-c("Recall","Precision","Cutoff","Split")
...
colnames(AUCs2)=paste(c("Train","Val","Test"),"AUC")

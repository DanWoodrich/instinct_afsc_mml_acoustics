...
dataPath <- args[4]
fillDat <- args[5]
randomize_order <- args[8]
stage_data <- args[9]
stage_prefix <- args[10]
...
if(stage_data != 'n'){
  ...
  dataPath = gsub(stage_data, stage_prefix, dataPath)
}
...
#change  all the paths for review. At the end, change them back. 
...
#use the max frequency in the ds to determine buffer height (want large enough to show up, not high enough to break)
if(nrow(Dets)>0){
  max_fq = max(Dets$HighFreq)
}else{
  max_fq = 5000
}
...
outNeg<-foreach(i=1:length(allFiles)) %do% {
  ...
  if(length(segVec)>0){
    outs<-foreach(f=seq(1,length(segVec),2)) %do% {
      segsRow<-c(segVec[f],segVec[f+1],0,max_fq,segs$FileName[1],segs$FileName[1],NA,"Not Considered",NA)
      return(segsRow)
    }
...
#if staging the data, copy it here. Note that the temp dir needs to be  self managed.  
if(stage_data != 'n'){
  
  FG_final= unique(c(out$`Begin Path`,out$`End Path`))
  
  FG_files_og = gsub(stage_prefix,stage_data,FG_final)
  
  dest_dirs <- unique(dirname(FG_final))
  sapply(dest_dirs, dir.create, recursive = TRUE, showWarnings = FALSE)
  
  #if already staged, don't overwrite. 
  file.copy(from = FG_files_og, to = FG_final, overwrite = FALSE)
}

...
#v1-7: 
#more sophisticated routine to determine detections. combination of smoothing and peak detection. 

args = "C:/Apps/INSTINCT/Cache/725729/402697/FileGroupFormat.csv.gz C:/Apps/INSTINCT/Cache/403160/829587/638915/598587/822849/363192 C:/Apps/INSTINCT/Cache/334512/741093/393863 C:/Apps/INSTINCT/Cache/403160/829587/638915/598587 C:/Apps/INSTINCT/Cache/403160/829587/638915/598587/856396 2400 40 300 45 65 240 240 65 0.05 within_file 20 moving-smooth-v1-7"
...
if(vertical_bins==1){
  ...
  for(i in 1:length(unique(FGs$Name))){
    ...
    for(p in unique(indFG$DiffTime)){
      ...
      new_size = round(length(scores)*(stride_pix/group_pix))
      ...
      #v1-7: here, use these time boundaries of scores to create new scores_starts and ends based on non-max suppression kind of 
      #algorithm. 
      ...
      group_secs = (stride_pix/group_pix)
      
      #populate vars
      bins_in=0
      start = scores_starts[1]+ model_s_size - group_secs
      probvec = c()
      splitsvec = c()
      
      for(z in 1:(length(scores)-1)){

        end = scores_ends[z] - model_s_size + group_secs
        
        bins_in = bins_in+1
        
        probvec = c(probvec,scores[z])
        splitsvec = c(splitsvec,splits[z])
        
        if((abs(scores[z+1]-scores[z])>smooth_prob)){ #use the smoothprob as a % of the prob so it is more sensitive at lower probs

          #if a single bin, just take the dimensions
          if(bins_in==1){
            start = scores_starts[z]
            end = scores_ends[z]
          }
          
          #if start is greater than end,swap them (test this..)
          if(start>end){
          ...
          if((end-start)<min_det_size){
            
            diff = min_det_size-(end-start)
            end = end + diff/2
            start = start - diff/2
            
          }
          
          newset[[detection_it]]=c(start,end,mean(probvec),min(splitsvec)) #min so that it gives preferentially to split where stats matter less
          
          detection_it= detection_it+1
          
          #reset variables
          bins_in =0
          start = scores_starts[z+1]+ model_s_size - group_secs
          probvec = c()
          splitsvec=c()
        }else if((end-start) > 25 | z==(length(scores)-1)){ #maximum detection size, keep the stats from getting too wonky. 
          ...
          #print("WARNING- mega huge detection")
          newset[[detection_it]]=c(start,end,mean(probvec),min(splitsvec)) #min so that it gives preferentially to split where stats matter less
      ...
      newset = do.call("rbind",newset)
      
      scores_starts=newset[,1]
      scores_ends=newset[,2]
      scores = newset[,3]
      splits = newset[,4]
      
      #crop the initial value so that it is >0
      ...
      detx = data.frame(scores_starts,scores_ends,freq_low,freq_low+freq_size,startfiles,endfiles,scores,FGdt$Name[1],splits)

        
      colnames(detx) = c("StartTime","EndTime","LowFreq","HighFreq","StartFile","EndFile","probs","FGID","splits")
      ...
      #if(any((detx$EndTime-detx$StartTime)>(model_s_size+50))){
      #  print(detx[which((detx$EndTime-detx$StartTime)>(model_s_size+50)),])
      #  stop("error found")
      #}
